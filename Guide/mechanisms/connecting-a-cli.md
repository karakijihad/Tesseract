...
drive its governed verb surface, so work done in a terminal lands in the same
memory, vault and workspace the assistant uses.

This page is for whoever is on the connecting end. Every verb call flows
through the same permission, cost and audit path as an in-process tool call;
no MCP route skips a gate.

TESSERACT also _is_ an MCP server — a spec-compliant Streamable-HTTP endpoint embedded in the Mirror backend, so a terminal-side agent (Claude Code CLI, Codex CLI, or any MCP client) can connect and drive the assistant's governed verb surface. This is the "Unreal Engine" model: the editor (Mirror) embeds the server; the CLI in the terminal is the occupant driving it. Every verb call flows through the same permission → cost → audit → ActivityRecord stack as an in-process tool call — no MCP path bypasses a gate.
...
Work belongs to the client that created it. A lane you open is owned by the principal your bearer token resolves to (`lane-claude`, `lane-codex`, `terminal-manual` for what TESSERACT spawns; `operator` for a tool you connected yourself) and another client cannot read it, send to it, turn it, attach to it, interrupt it or close it. `activity_list` and the SSE stream show you your own records and nothing else. The check runs in the controller daemon, not in the Mirror, so it holds on the raw IPC path too — Mirror-side filtering would be usability, not a boundary.
...
**First, the surface has to be on.** TESSERACT ships with it switched off — it is the one door through which another program reaches everything the assistant knows, so it opens deliberately. The switch is in **Settings → Keys → MCP** and takes effect on the next start. Until then `/mcp` answers that it is off, and that applies to the CLIs in TESSERACT's own terminal as much as to anything outside.

With it on, registration is automatic and nobody has to type it. Opening a terminal pane, starting a lane, or running a delegate provisions both CLIs at **user scope** — `~/.claude.json` (`mcpServers.tesseract`) and `~/.codex/config.toml` (`[mcp_servers.tesseract]`). Neither file records a secret; both reference the bearer token by environment-variable name.

The tokens those names point at are TESSERACT's own, and it mints them for itself on first start. `lane-claude`, `lane-codex` and `terminal-manual` are identities the runtime issues to processes it spawns, not credentials anyone types — which is why they appear in no settings screen.
...
```bash
export TESSERACT_MCP_SECRET=<the operator token — Settings → Keys → MCP mints one and shows it once>
