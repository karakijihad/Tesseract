...
def _resolve_model_ref(model_role: str) -> str | None:
...
def _serialize_agent(agent: AgentDefinition, *, status: str) -> dict[str, Any]:
    return {
        "name": agent.name,
        "description": agent.description,
        "version": agent.version,
        "model_role": agent.model_role,
        "resolved_ref": _resolve_model_ref(agent.model_role),
...
async def list_agents_handler(request: web.Request) -> web.Response:
    ...
    for name in list_agents():
        try:
            ...
            continue
        out.append(_serialize_agent(agent, status="active"))
    return web.json_response({"agents": out, "errors": errors})
...
async def list_pending_handler(request: web.Request) -> web.Response:
    ...
    out: list[dict[str, Any]] = []
    errors: list[str] = []
    for name in list_pending_agents():
        try:
            agent = load_agent(name, include_pending=True)
        except Exception as exc:
            errors.append(f"{name}: {exc}")
            continue
        out.append(_serialize_agent(agent, status="pending"))
    return web.json_response({"agents": out, "errors": errors})
...
async def save_agent_source_handler(request: web.Request) -> web.Response:
    ...
    try:
        path = save_agent_source(name, source)
    except FileNotFoundError:
        return web.json_response({"error": f"agent {name!r} not found"}, status=404)
    except RuntimeError as exc:
        return web.json_response({"error": str(exc)}, status=400)
    return web.json_response({"name": name, "path": str(path), "saved": True})
