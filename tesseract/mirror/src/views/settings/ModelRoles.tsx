...
function optionLabel(entry: CatalogEntry): string {
  ...
  const ctx = entry.context_window ? `  ${Math.round(entry.context_window / 1000)}k` : '';
...
export function ModelRolesSection() {
  ...
  const toggleMode = async (target: ModelRefTarget, currentMode: 'active' | 'disabled') => {
    setSavingTarget(target);
    setError(null);
    try {
      ...
      setError(err instanceof Error ? err.message : 'role-models update failed');
    } finally {
      setSavingTarget(null);
    }
  };
  ...
  return (
    <section className="settings-section">
      ...
      <div className="role-banner" role="status">
        Takes effect immediately — live sessions hot-swap to the new adapter on save.
      </div>
      ...
      <div className="role-table">
        ...
        {targetRows.map(({ target, allow_toggle, load_bearing, mode: serverMode }) => {
          const label = target;
          ...
          const status: IdentityRoleStatus | undefined = allow_toggle
            ? roles?.[target]
            : undefined;
          const mode = (status?.mode || serverMode || 'active') as 'active' | 'disabled';
          ...
          return (
            <div key={target} className="role-table__row">
              ...
              <span className="role-table__ctx t-meta">{ctxLabel(head?.context_window)}</span>
