...
export const useWebSocketStore = create<WebSocketState>((set, get) => {
  ...
  async function _catchUp(sessionId: string | null, lastEventTs: string | null): Promise<void> {
  ...
  return {
    ...
    connect: () => {
      ...
      if (
        _socket !== null &&
        (_socket.readyState === WebSocket.CONNECTING || _socket.readyState === WebSocket.OPEN)
      ) return;
        ...
        if (!_autoResumeAttempted) {
          ...
          if (persistedName && _socket?.readyState === WebSocket.OPEN) {
            ...
            void Promise.all([
              ...
              const match = sessions.find((s) => s.session_id === persistedName);
      ...
      ws.onmessage = (event: MessageEvent) => {
        ...
        if (
          ...
          useTerminalStore.getState().handleRawMessage(raw as Record<string, unknown>);
          return;
        }
        ...
        } else if (rec && rec.channel === 'activity' && typeof rec.kind === 'string') {
      ...
      ws.onclose = (event) => {
        ...
        if (next >= MAX_RECONNECT_BEFORE_ERROR) {
          useEntityStore.getState().setState('error');
        }
