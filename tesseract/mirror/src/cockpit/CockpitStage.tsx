...
import { restoreLanes } from "../canvas/triorenderer";
...
// SC-1/2/3 — the fixed spatial cockpit. The TARS orb (GlobalCanvas, full-screen
...
// `data-view="tars"` mode — atmosphere + glass HUD — so the orb keeps the exact
// rendering it had on the old TarsView.
...
export function CockpitStage() {
  ...
  useEffect(() => {
    ...
    void restoreLanes("tars").catch(() => undefined);
  }, [ensureRails]);

  return (
    <div className="cockpit-shell" data-view="tars">
      <TopStatusHud />
      <main className="cockpit-center">
        <div className="cockpit-stage">
          <OrbAnchor />
          {/* Ambient captions of TARS's latest line, faded under the orb —
              voice-first "what TARS just said" without opening the Chat tab.
          ...
          <ParkedAsksPane />
          {/* SC-4 — TARS-spawned Surface-Protocol cards (CV-1 lanes + routing,
