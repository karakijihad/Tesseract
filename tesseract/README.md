...
The runtime — memory, permissions, tool registry, session store, adapters. The operator-facing assistant persona runs inside it; its name is config (`config/mirror.yaml::identity.name`), not a constant.
...
This directory is slated to become its own clean repository once the current reboot + Mirror revival stabilise. Target state:

- **Repo name:** `tesseract` (runtime) or `agent` (operator persona) — TBD by operator.
