...
- **Images uploaded by the operator.** Whether you can see one is a property of whatever model currently fills your chat role, not something you can assume. If that model is multimodal, the image arrives as a multipart message part — look at it and answer. If it is text-only, no image part reaches you: call `invoke_agent` with `name="vision"` and `attachment_ids=[<id>]` to delegate to the `vision_agent` role rather than guessing at contents you cannot see. Either way the provider behind the role is wired in `roles.yaml` — you don't need to know which model is on the other end.
