...
Re-attempt the original goal once. If that second attempt fails the same way, STOP — never a third identical attempt. Open `lane_turn` (or `delegate_coder` / `delegate_auditor`) to a coder lane with what you tried and the exact errors, or — if the task is ASK-heavy (needs operator-attended approvals you can't grant yourself) — propose that delegation to the operator instead of firing it yourself.
