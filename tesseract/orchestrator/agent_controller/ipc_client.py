...
class ControllerClient:
    ...
    async def request_snapshot(
        ...
        await self._send(
            {"msg": "activity_snapshot", "caller_principal": caller_principal}
        )
    ...
    async def shutdown(self) -> None:
        ...
        spawn a fresh daemon. Used by ``agent`` on default Ctrl+C exit
    ...
    async def lane_open(
        ...
        *,
        ...
        shared_with: list[str] | tuple[str, ...] = (),
    ) -> str:
    ...
    async def lane_send(
        self, lane_id: str, message: str, *, caller_principal: str | None = None
    ) -> dict[str, Any]:
    ...
    async def lane_turn_read(
        ...
        *,
        ...
        """One turn-scoped poll. Returns ``{"known": bool, "outcome": {...}}``
        — `known=False` says the lane never issued that turn, which the
        caller must treat as fatal rather than falling back to "whichever
        turn ends first"."""
        return await self._lane_call(
            {
                "msg": "lane_turn_read",
    ...
    async def lane_status(
        self, lane_id: str, *, caller_principal: str | None = None
    ) -> dict[str, Any]:
        return await self._lane_call(
            {
                "msg": "lane_status",
    ...
    async def lane_interrupt(
        ...
        *,
        ...
        # Cancel a turn (steer) without closing the lane. `turn_id` scopes it
        # to one turn; omitted, it cancels whichever turn is busy.
        return await self._lane_call(
            {
                "msg": "lane_interrupt",
                "lane_id": lane_id,
    ...
    async def lane_list(
        self, *, caller_principal: str | None = None
    ) -> list[str]:
        payload = await self._lane_call(
            {"msg": "lane_list", "caller_principal": caller_principal}
        )
    ...
    async def lane_named_get(
        self, name: str, *, caller_principal: str | None = None
    ) -> dict[str, Any] | None:
    ...
    async def lane_named_list(
        self, *, caller_principal: str | None = None
    ) -> list[dict[str, Any]]:
        payload = await self._lane_call(
            {"msg": "lane_named_list", "caller_principal": caller_principal}
        )
