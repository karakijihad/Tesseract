...
class _SessionHandlersMixin:
    ...
    async def _on_new_session(
        ...
        record = self._registry.create_session(
            ...
            preferred_seat=msg.preferred_seat,
            owner_principal=msg.owner_principal,
        )
    ...
    async def _on_activity_snapshot(
        ...
        from .lanes.principals import OPERATOR_PRINCIPAL, is_known_principal

        # Scoped like `activity.list`, for the same reason: a snapshot that
        # returns every principal's records is the enumeration the MCP-side
        # filter withholds, reachable one layer down. A blank owner is the
        # runtime's own work, which is the operator's.
        caller = (msg.caller_principal or "").strip()
        if not caller or not is_known_principal(caller):
            # Same rule as the lane handlers, deliberately: defaulting an
            # absent caller to the operator would make every message that
            # forgot the field an administrative one.
            await conn.outbound.put(
                ActivitySnapshotPush(records=[]).model_dump(mode="json")
            )
            return
        records = [
            r.model_dump(mode="json")
            for r in get_activity_registry().snapshot()
            if caller == OPERATOR_PRINCIPAL
            or (r.owner_principal and r.owner_principal == caller)
        ]
        await conn.outbound.put(
