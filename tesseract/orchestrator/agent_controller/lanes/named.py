...
from .principals import OPERATOR_PRINCIPAL, may_reach
...
def _authorize_open(name: str, *, read_only: bool, caller: str) -> None:
    """Refuse a writeable named lane to anyone but the operator.

    `lane.ensure` is AUTO for every trusted MCP client and is the only
    lane-opening verb on that surface, so without this a client can mint itself
    a name and receive a lane spawned WITHOUT the sandbox flag. Refusing states
    ...
    """
    if read_only or caller == OPERATOR_PRINCIPAL:
        return
    raise NamedLaneError(
        f"{caller!r} may not open writeable lane {name!r}: an MCP client may "
        "open read-only named lanes only. The operator can open it themselves."
    )


def _resolve_working_dir(
    working_dir: str | None, *, name: str, recorded: str | None = None
) -> str:
    """An explicit `working_dir` wins, then the binding's recorded one, then
    the active project.

    Refusing when none of the three is available is the point of the default. A
    lane silently inheriting whichever directory the backend process happened
    to start in is the ambiguity this closes — a wrong-but-plausible cwd is
    worse than being told to pick one, because the work lands somewhere nobody
    chose.
    """
    # Stripped, not just truthy. `"   "` is truthy in Python, so a bare
    # `if working_dir:` hands three spaces to the spawn as a cwd and the caller
    # gets an opaque OS error instead of the default. The Mirror route rejects
    # a blank value outright; every other caller reaches this line, so the rule
    ...
    except Exception as exc:  # noqa: BLE001 — a broken registry is not a lane error
        raise NamedLaneError(
            f"named lane {name!r} was opened with no working_dir and the "
            f"project registry could not be read ({exc}). Pass working_dir "
            "explicitly."
        ) from exc
    if project is None:
        raise NamedLaneError(
            f"named lane {name!r} was opened with no working_dir and no "
...
class NamedLaneManager:
    ...
    async def ensure(
        ...
        *,
        ...
        `working_dir=None` means "the active project's root", resolved on the
        open branch only — a reuse answers from the binding's own recorded
        directory, which is both already correct and the one an attach after a
        restart will use. Resolution lives here rather than in each caller so
        the in-process and IPC paths cannot disagree about where a lane lives,
        and the record carries the RESOLVED path so a later project switch does
        not move a lane that is already open."""
        _validate_name(name)
        lock = self._name_locks.setdefault(name, asyncio.Lock())
        async with lock:
            existing = read_named_lane(name)
            self._authorize_binding(existing, owner_principal)
            if existing is not None:
                ...
                if self._is_lane_alive(existing.lane_id) and self._posture_matches(
                    ...
                    register_lane(
                        bumped.lane_id,
                        label=bumped.name,
                        provider=bumped.kind,
                        owner_principal=self._owner_of(bumped.lane_id),
                        shared_with=self._shared_with_of(bumped.lane_id),
                    )
                    return bumped

            # Everything below opens a process. Both gates live here, inside
            # the lock and past the reuse branch, so a caller driving a lane it
            # already owns meets neither.
            _authorize_open(
                name, read_only=read_only, caller=owner_principal
            )

            # Only the open branch has to choose a directory. A replacement
            # inherits the one its binding already recorded unless the caller
            # named a new one — the lane is the same lane by name, and moving
            # it because the operator switched projects is not what `ensure`
            # was asked to do.
            resolved_dir = _resolve_working_dir(
                working_dir,
                name=name,
                recorded=existing.working_dir if existing is not None else None,
            )
    ...
    def _authorize_binding(
        self, existing: NamedLaneRecord | None, caller: str
    ) -> None:
        """Refuse an `ensure` against a binding the caller does not own.

        `lane.ensure` is AUTO for every trusted client, so both of `ensure`'s
        branches are reachable by a peer, and each leaks differently. The reuse
        branch hands back the record — lane_id, working_dir, model — which is
        the enumeration the owner check exists to withhold. The replace branch
        is sharper still: it CLOSES the existing lane on the way to opening a
        fresh one, so an unchecked peer does not merely read the binding, it
        destroys the lane behind it.

        Checked once, before either branch, and against the LANE's persisted
        owner rather than anything on the binding record — the lane is what
        carries authority; the binding is a name pointing at it. An unbound
        name is open to whoever asks for it first.
        """
        if existing is None:
            return
        owner = self._owner_of(existing.lane_id)
        if not may_reach(caller=caller, owner=owner, shared_with=[]):
            raise NamedLaneError(
                f"named lane {existing.name!r} is bound to a lane owned by "
                f"{owner!r}; {caller!r} may not ensure it"
            )

    def _shared_with_of(self, lane_id: str) -> tuple[str, ...]:
        try:
            return tuple(read_lane(lane_id).shared_with)
        except (FileNotFoundError, OSError, ValueError):
            return ()

    def _owner_of(self, lane_id: str) -> str:
        """The reused lane's persisted owner. Re-registering under the human
        name must not drop it back to unowned — the activity snapshot is what
        `activity.list` scopes on, and a blank owner reads as the operator's,
        which would hide a client's own lane from it."""
        try:
            return read_lane(lane_id).owner_principal
        except (FileNotFoundError, OSError, ValueError):
