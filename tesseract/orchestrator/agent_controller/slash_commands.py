"""In-session ``/command`` parser + dispatch for the ``agent`` TUI.

The stdin loop in :mod:`tesseract.scripts.agent_cli` consults
...
if TYPE_CHECKING:  # pragma: no cover — typing only
    from tesseract.scripts.agent_cli import _TuiSession
...
async def _cmd_new(tui: "_TuiSession", args: str) -> None:
    ...
    tui.renderer.render_notice(
        ...
        f" — run `agent --session {new_id}` to attach",
        style="green",
    )
