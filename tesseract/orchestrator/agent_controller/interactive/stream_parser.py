...
class ClaudeTurnAccumulator:
    ...
    When the turn was launched with `--json-schema`, that same result event
    also carries `structured_output` — the validated object. It is captured
    separately rather than read off `result`: `result` is a *string* holding
    the serialised JSON, so a caller reading it would be re-parsing what the
    CLI already parsed and validated.
    """
...
class CodexTurnAccumulator:
    ...
    Under `--output-schema` codex emits no separate structured field: the
    validated JSON arrives *as* the final agent_message text, so `result_text`
    is the structured value and a caller parses it. This differs from Claude,
    which carries a already-parsed `structured_output` object alongside the
    text — the two are left un-unified on purpose (the relay is the right place
    for one accessor, once both paths have been exercised).
    """
