...
* **Persona tags** — ``<answer>`` / ``</answer>`` get stripped (the
  ...
  is dimmed so "TARS thinking out loud" is visually distinct from the
  actual answer.

The renderer still exposes the same ``render(event)`` / ``render_header``
contract so callers (tars_cli, replay paths, tests) don't have to
...
# so the operator can tell "TARS thinking" from "TARS answering" at a
...
# for ``delegate_claude``); the panel shows a one-line preview and the
...
HEADER = """\
╔══════════════════════════════════════╗
║   TESSERACT · TARS terminal client   ║
╚══════════════════════════════════════╝
"""
...
class TuiRenderer:
    ...
    * ``record=True`` — captures all output to an in-memory buffer
    ...
    def _assistant_text(self, event: dict[str, Any]) -> None:
        ...
        worker detail pane rather than as a main-transcript TARS bubble
        ...
        if not self._assistant_streaming:
            self._console.print(Text("▍tars ", style="bold magenta"), end="")
    ...
    def _cli_chunk(self, event: dict[str, Any]) -> None:
        """Live stdout from ``delegate_claude``/``delegate_codex``
