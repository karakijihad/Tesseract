...
hand work to a TARS controller session.

One primitive, many callers:

* Autonomy kernel runner (via ``delegate_tars_controller`` tool)
* Mirror chat brain (via ``start_controller_session`` tool)
* Scheduler / workspace cards / anything else with work to dispatch
* The ``tars`` CLI itself (uses :func:`ensure_daemon_running` to
...
1. ``ensure_daemon_running()`` — probes ``controller.port`` + a TCP
   connect; if dead, spawns ``python -m tesseract.scripts.tars_controller``
   ...
   alive (the supervisor or another ``tars`` window already booted it).
2. :func:`dispatch_to_controller` — opens an IPC client, mints a session
   ...
   * ``wait_for_completion=False`` — returns the session_id immediately
     ...
     own parent transcript and an operator can ``tars --session <id>``
     to attach later.
...
:data:`tars_controller.sessions.SessionOrigin`. Each surface owns its
...
"""Session mode — see :data:`tars_controller.sessions.SessionMode`."""
...
def _spawn_daemon_subprocess() -> subprocess.Popen:
    """Spawn ``python -m tesseract.scripts.tars_controller`` as a
    detached sibling. Matches the supervisor's spawn pattern so the
    daemon survives the spawning process exiting (the ``tars`` CLI
    ...
    into DEVNULL.
    """
    cmd = [sys.executable, "-m", "tesseract.scripts.tars_controller"]
    env = os.environ.copy()
...
def _try_acquire_spawn_lock() -> bool:
    ...
    a time. Uses ``O_CREAT | O_EXCL`` so two concurrent ``tars``
...
async def ensure_daemon_running(
    *,
    ...
    ``tars`` calls + a supervisor boot don't all try to spawn three
...
def _resolve_default_coder() -> str | None:
    """Read the soft ``coder_default`` policy from roles.yaml. An absent or
    invalid key resolves to ``None`` — the session keeps both delegate
    tools and the controller brain picks. A config-load error must never
    fail a dispatch, so it too collapses to ``None``."""
    try:
        from tesseract.config.loader import load_config

        value = load_config().roles_raw.get("coder_default")
    except Exception:  # noqa: BLE001 — config errors must not break dispatch
        return None
...
async def dispatch_to_controller(
    ...
    * Workspace card: ``origin="mirror"`` (closest semantic — the card
    ...
    # None from the caller means "use the YAML default". Resolve here so
    # callers never need to know the policy; the resolved value flows to
    # the session record via new_session.
    if preferred_coder is None:
        preferred_coder = _resolve_default_coder()
