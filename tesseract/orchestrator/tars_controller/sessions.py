...
and both must be readable / writable from the controller daemon, the tars
TUI, and the Mirror backend:

* `ControllerSessionRecord` — durable identity for one controller
  session. Lives at
  `<TESSERACT_HOME>/tars_controller/sessions/<session_id>.json`.
...
CoderChoice = Literal["claude", "codex"]
...
class ControllerSessionRecord(BaseModel):
    ...
    preferred_coder: CoderChoice | None = None
...
class SessionRegistry:
    ...
    def create_session(
        ...
        *,
        ...
        register_session(
            record.session_id, label=record.title or record.mode, status=record.status
        )
        return record
