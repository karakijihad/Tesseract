"""TC-4 — headless TARS controller daemon.

A sibling process to the Mirror backend. Owns:

* `<TESSERACT_HOME>/tars_controller/controller.json`  — singleton PID record
* `<TESSERACT_HOME>/tars_controller/<controller_id>/heartbeat` — fresh-touch every 30s
...
* `<TESSERACT_HOME>/tars_controller/sessions/<session_id>.json` — controller-session ledger
* `<TESSERACT_HOME>/tars_controller/transcripts/<session_id>.jsonl` — typed event stream
