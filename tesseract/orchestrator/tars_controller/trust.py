"""First-run trust prompt for the ``tars`` CLI.
...
same so the ``tars`` UX matches operator expectations.
...
def prompt_trust(
    ...
    *,
    ...
    banner = (
        "\nTARS hasn't been run from this directory before:\n"
        f"  {target_str}\n"
        "Trust this directory and let TARS read/write files here? [y/N]: "
    )
    try:
