"""Textual TUI for ``agent`` — the visual upgrade.

The old ``agent_cli`` writes ANSI strings to stdout. That works on any
...
* :class:`AgentApp` — the root :class:`textual.app.App`. Owns the
...
``agent --list`` and tests usable without spinning a full terminal app.
"""
...
# Light-grey italic for <intent> announcements ("the assistant is about to do X").
...
# "delegate_coder spawned in background: handle=del-claude-20260524-…".
...
class ThinkingIndicator(Static):
    """Transient 'the assistant is working' sign (operator-requested 2026-05-26).
...
class BackgroundJobDetail(ModalScreen[None]):
    ...
    def refresh_from_app(self) -> None:
        app: AgentApp = self.app  # type: ignore[assignment]
...
class AgentApp(App[int]):
    """Root Textual app for the ``agent`` CLI.
    ...
    * 130 — Ctrl+C / interrupt
    ...
    def compose(self) -> ComposeResult:
        ...
        yield Horizontal(
            ...
            Input(
                placeholder="ask the agent…   /help  :approve  :deny  :quit",
                id="input",
            ),
            id="input-row",
        )
    ...
    async def on_mount(self) -> None:
        # The assistant branding in the Header (App.title is what Header paints).
        self.title = "the assistant"
        sid = self._session_id[-12:] if self._session_id else "?"
        ...
        self._push_task = asyncio.create_task(
            self._push_loop(), name="agent-app-push"
        )
        self._tick_timer = self.set_interval(
            0.5, self._on_tick, name="agent-app-tick"
        )
    ...
    async def _push_loop(self) -> None:
        try:
            ...
            log.exception("agent app: push loop crashed: %s", exc)
    ...
    async def _slash_new(self, args: str) -> None:
        ...
        await self._notice(
            f"+ minted session {new_id} — run `agent --session {new_id}` to attach",
            style="green",
        )
    ...
    async def _slash_delete(self, args: str) -> None:
        ...
        if target == self._session_id:
            await self._notice(
                "/delete cannot remove the attached session — run /quit, then `agent --delete <id>`",
                style="red",
            )
            return
