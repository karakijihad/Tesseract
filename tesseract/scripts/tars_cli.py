"""TC-6 — `tars` terminal client entry point.

Operator types `tars` in any shell; the script connects to the running
...
* `tars`                   — interactive picker over active sessions
* `tars --new`             — mint a new session and attach
* `tars --session <id>`    — attach to a specific id
* `tars --list`            — list active sessions and exit
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="tars",
        description="TARS terminal client — attach to the controller daemon",
    )
    parser.add_argument(
        "--session",
...
def _get_prompt_session() -> Any | None:
    ...
    try:
        ...
        log.debug("tars: prompt_toolkit unavailable; falling back to stdin",
                  exc_info=True)
    ...
    async def push_loop(self) -> None:
        async for payload in self.client.pushes():
            ...
            if event_name == "session_deleted":
                ...
                # an out-of-band deletion (Mirror UI, second tars window)
                # can still trip this.
            ...
            log.debug("tars: drop push event %s", event_name)
...
async def _drive_session(
    ...
    *,
    ...
    stdin_task = asyncio.create_task(tui.stdin_loop(), name="tars-stdin")
    push_task = asyncio.create_task(tui.push_loop(), name="tars-push")
    try:
...
async def _async_main(args: argparse.Namespace) -> int:
    ...
    if not skip_trust and not is_trusted(os.getcwd()):
        ...
        if not approved:
            print("tars: directory not trusted; aborting", file=sys.stderr)
            return 3
    ...
    try:
        ...
        print(f"tars: could not start controller daemon: {exc}", file=sys.stderr)
        return 2
    ...
    try:
        ...
        print(f"tars: {exc}", file=sys.stderr)
        return 2
    ...
    async with client:
        ...
        if args.delete:
            try:
                ...
                print(f"tars: delete failed: {exc}", file=sys.stderr)
                return 2
...
async def _drive_session_textual(
    ...
    *,
    ...
    construct ``TarsApp`` with the live client, and run until the app
    exits (Ctrl+C / ``:quit`` / disconnect).
    """
    from tesseract.scripts.tars_app import TarsApp
...
def _delete_session_offline(session_id: str) -> int:
    ...
    Used by ``tars --delete`` when no daemon is running — sessions are
...
async def _shutdown_running_daemon() -> int:
    ...
    try:
        ...
        print(f"tars: shutdown send failed: {exc}", file=sys.stderr)
        return 2
    # Brief wait for the daemon's port-file unlink so a subsequent
    # `ensure_daemon_running` doesn't observe the stale port.
    from tesseract.orchestrator.tars_controller.paths import port_file_path
