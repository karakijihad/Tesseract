"""WorkerLivenessJob — notice a worker that stopped beating, while the app
is still running.
...
This job closes that gap by reading only what is already on disk: a worker in
a non-terminal status whose heartbeat has gone stale is logged at WARNING,
and this module is on the log forwarder's `_ELEVATED_LOGGERS` list so that
...
every tick — is how a pulse feed becomes something the operator learns to
ignore, which is the same failure this and P17 exist to prevent.
"""
...
# Worker ids already announced this process. Module-level so it survives the
# scheduler rebuilding the job object between runs.
...
class WorkerLivenessJob(BaseJob):
    async def run(self, ctx: JobContext) -> JobResult:
        t0 = time.monotonic()
        threshold = float(
            ctx.config.get("staleness_threshold_seconds", STALENESS_THRESHOLD_SECONDS)
        )
        now = time.time()

        checked = 0
        stale: list[str] = []
        announced_now: list[str] = []
        ...
        for record in list_active_records():
            ...
            checked += 1
            if not is_heartbeat_stale(record.id, now=now, threshold=threshold):
                continue
            stale.append(record.id)
            if record.id in _announced:
                continue
            _announced.add(record.id)
            announced_now.append(record.id)
            mtime = read_heartbeat_mtime(record.id)
            silent_for = "never beat" if mtime is None else f"{now - mtime:.0f}s ago"
            log.warning(
        ...
        # Forget workers that are no longer live, so this set cannot grow
        # without bound. It must filter by STATUS, not by presence in
        # `active/`: `KernelWorkerRunner` writes a terminal status but never
        # calls `archive_record` (only cancel and recovery do), so a normally
        # completed worker sits in `active/` indefinitely and an
        # unfiltered scan would keep re-adding it forever.
        ...
        return JobResult(
            ...
            ok=True,
            detail=(
                f"{checked} live worker(s), {len(stale)} stale, "
                f"{len(announced_now)} newly announced"
            ),
            payload={
                "checked": checked,
                "stale": stale,
                "announced": announced_now,
