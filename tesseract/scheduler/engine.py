...
class SchedulerEngine:
        ...
        async def _gated_catchup(name: str, rt: _JobRuntime) -> None:
            try:
                async with catchup_sem:
                    ...
                    await self._run_job(name, rt, now, catchup=True)
            finally:
    ...
    def add_job_runtime(
        ...
        *,
        ...
        and `on_failure ∈ {log, alert, disable}`. Persists to
        `schedule.yaml` then arms in `self.registry`.
        """
        ...
        # than silently parking a placeholder job.
    ...
    async def run_now(self, name: str) -> JobResult:
        ...
        return await self._run_job(name, rt, fired_at, catchup=False)
    ...
    async def _tick(self, now_utc: datetime, now_local: datetime) -> None:
        for name, rt in self.registry.items():
            if not rt.enabled:
                continue
            if name in self._catchup_pending:
                continue
            if not self._should_fire(rt, now_utc, now_local):
                continue
            rt.last_fired_at = now_utc
            self.spawn_tracked_task(
                self._run_job(name, rt, now_utc), name=f"scheduler-{name}"
            )
